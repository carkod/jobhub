...
class Children extends Component {
  ...
  componentDidUpdate = (p) => {
    if (this.props.children !== p.children)
      this.setState({
      ...
      <>
        <List relaxed>
          ...
          <Input
            icon="tags"
            iconPosition="left"
            label={{ tag: true, content: newButton(singLabel) }}
            labelPosition="right"
            placeholder={`New ${singLabel}`}
            onChange={this.newChild}
            ...
            {children.length > 0 &&
              children.map((it, i) => (
                <Form.Group key={it.key} widths="equal">
                  <Button onClick={this.removeChild(i)} color="red" icon>
                    <Icon name="remove" />
                  </Button>
                  <Input
                    label={"Text: "}
