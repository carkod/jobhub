...
class Relationships extends Component {
  ...
  componentDidUpdate = (p) => {
    if (this.props.relations !== p.relations)
  ...
  save = (children) => {
    this.setState(
      ...
      () =>
        this.props.saveRelationApi(
          this.state.relations[this.state.activeIndex],
        ),
    );
  };
  ...
  render() {
    ...
    if (this.state.relations.length > 0) {
      const arrayList = this.state.relations.map((cv, i) => ({
        key: cv._id || shortid.generate(),
        title: {
          content: (
            <span color={this.state.savedID === cv._id ? "red" : "inherit"}>
              {cv.title}
            </span>
          ),
        },
            ...
            <div className="metadata">
              <div className="meta-content">
                <List horizontal relaxed>
                  <List.Item className="u-fields-spacing">
                    label:{" "}
                    <Input
                      value={cv.label}
                      name="label"
                  ...
                  <List.Item className="u-fields-spacing">
                    title: <Input value={cv.title} name="title" />
                  </List.Item>
                </List>

                <Children
                  singLabel={cv.singLabel}
...
export default connect(mapStateToProps, { fetchRelationsApi, saveRelationApi })(
  Relationships,
);
