...
class Portfolio extends Component {
  ...
  render() {
    const { portfolio } = this.state;
    const list = portfolio.map((proj, i) => ({
      key: proj._id || shortid.generate(),
      title: {
        content: (
          <span color={this.state.savedID === proj._id ? "red" : "inherit"}>
            {proj.name}
          </span>
        ),
      },
          ...
          <div className="metadata">
            <div className="meta-content">
              <List horizontal relaxed>
                <List.Item>
                  <Icon fitted name="id card" /> {proj._id || "N/A"}
                </List.Item>
                <List.Item>
                  <Icon fitted name="checked calendar" />{" "}
                  {moment(proj.updateDate).format("Do MMMM YYYY") || "N/A"}
                </List.Item>
                <List.Item>
                  <Icon fitted name="clock" />{" "}
                  {moment(proj.createdDate).format("Do MMMM YYYY") || "N/A"}
                </List.Item>

                <List.Item>
                  <Icon fitted name="briefcase" />{" "}
                  {proj.cats ? proj.cats.position : "N/A"}
                </List.Item>
                <List.Item>
                  <Icon fitted name="talk" />{" "}
                  {proj.cats ? proj.cats.cvLang : "N/A"}
                </List.Item>
                <List.Item>
                  <Icon fitted name="globe" />{" "}
                  {proj.cats ? proj.cats.cvCountry : "N/A"}
                </List.Item>
              </List>
            </div>
            <div className="buttons">
              <Link
                ...
                to={`/portfolio/project/id=${proj._id}`}
              >
                Edit/View
              </Link>
    ...
    return (
      <div id="portfolio" className="">
        <h1>
          Section - All projects <NewProject />
        </h1>
